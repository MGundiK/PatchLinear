...
run3() {
  ...
  for SEED in 2021 2022 2023; do
    ...
    if [[ -z "$line" ]]; then
      echo "seed=${SEED} mse: NA mae: NA (missing from log)"
      continue
    fi

    local mse mae
    mse=$(echo "$line" | awk '{for(i=1;i<=NF;i++) if($i=="mse:") print $(i+1)}')
    mae=$(echo "$line" | awk '{for(i=1;i<=NF;i++) if($i=="mae:") print $(i+1)}')

    if [[ -z "$mse" || -z "$mae" ]]; then
      echo "seed=${SEED} mse: NA mae: NA (parse failed)"
      continue
    fi

    echo "seed=${SEED} mse: ${mse} mae: ${mae}"

    mse_sum=$(awk -v a="$mse_sum" -v b="$mse" 'BEGIN{printf "%.10f", a+b}')
    mae_sum=$(awk -v a="$mae_sum" -v b="$mae" 'BEGIN{printf "%.10f", a+b}')
    n=$((n+1))
    ok_runs=$((ok_runs+1))
  done

  if [[ "$ok_runs" -eq 0 ]]; then
    echo "NA NA ${TAG}" | tee -a "$RESULT"
    return 0
  fi

  local mean_mse mean_mae
  mean_mse=$(awk -v s="$mse_sum" -v n="$n" 'BEGIN{printf "%.6f", s/n}')
  mean_mae=$(awk -v s="$mae_sum" -v n="$n" 'BEGIN{printf "%.6f", s/n}')

  echo "avg mse: ${mean_mse}  avg mae: ${mean_mae}"
  echo "${mean_mse} ${mean_mae} ${TAG}" | tee -a "$RESULT"
  echo ""
}
...
for D in 16 32 48 64 96 128 192; do
  T=$((D*2))
  run3 "d${D}_t${T}_c16_cc1_ag1_lr001" $FIXED \
...
for DEC in 0 1; do
  for SEAS in 0 1; do
    for FUS in 0 1; do
      [ "$DEC" -eq 0 ] && [ "$SEAS" -eq 0 ] && [ "$FUS" -eq 0 ] && continue
      if [ "$SEAS" -eq 0 ]; then
...
echo ""
echo "=== TOP 15 CONFIGS BY MSE ==="
grep '^[0-9]' "$RESULT" | sort -n | head -15 | tee -a "$RESULT"
...
echo ""
echo "Full results saved to: $RESULT"
