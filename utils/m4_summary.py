...
def group_values(values, groups, group_name):
    return np.array(
        [v[~np.isnan(v)] for v in values[groups == group_name]],
        dtype=object
    )
...
class M4Summary:
    ...
    def evaluate(self):
        ...
        Skips any frequency whose forecast CSV has not been generated yet,
        so partial evaluation (e.g. after a single frequency run) works correctly.

        :return: sMAPE, OWA, MAPE, MASE grouped by seasonal patterns.
        """
        grouped_owa = OrderedDict()

        naive2_forecasts = pd.read_csv(self.naive_path).values[:, 1:].astype(np.float32)
        naive2_forecasts = np.array(
            [v[~np.isnan(v)] for v in naive2_forecasts], dtype=object
        )
        ...
        for k in grouped_model_mases.keys():
            if k in grouped_naive2_mases and grouped_naive2_mases[k] != 0:
                grouped_owa[k] = (
    ...
    def summarize_groups(self, scores):
        ...
        Skips missing frequencies gracefully (for partial evaluation).
        """
        ...
        if weighted_score:
            total_count = sum(group_count(g) for g in scores)
            scores_summary['Average'] = (
                np.sum(list(weighted_score.values())) / total_count
            )
